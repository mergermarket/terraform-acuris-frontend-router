...
module "default_backend_ecs_service" {
  ...
  name   = format("%s-%s-404", local.simple_env, var.component)
...
module "alb" {
  ...
  internal                 = "false"
  certificate_domain_name  = var.dev_subdomain ? format("*.%s%s", local.simple_env != "live" ? "dev." : "", var.alb_domain) : "*.${var.alb_domain}"
